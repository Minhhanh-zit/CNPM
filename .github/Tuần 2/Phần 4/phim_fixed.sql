/* =========================================================
   ...
   BẢN SỬA: Bổ sung session/JWT, OTP, combo, voucher,
            ...
            thanh toán mở rộng và quy trình ISSUED.
   ========================================================= */
...
BEGIN
    ...
    BEGIN TRY
        ...
        VALUES (
            ...
            @showtime_seat_id,
            CONCAT('HOLD-', NEWID()),
            DATEADD(MINUTE, @hold_minutes, GETDATE()),
            'ACTIVE'
        );
...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            RAISERROR(N'Ghế giữ không tồn tại hoặc đã hết hạn.', 16, 1);
            ROLLBACK TRANSACTION;
            RETURN;
        END;
        ...
        IF @showtime_id IS NULL
        BEGIN
            RAISERROR(N'Ghế không ở trạng thái HELD.', 16, 1);
            ROLLBACK TRANSACTION;
            RETURN;
        END;
...
BEGIN
    ...
    BEGIN TRY
        ...
        ;WITH hold_list AS (
            SELECT DISTINCT TRY_CAST(value AS INT) AS hold_id
        ...
        SELECT
            @hold_count = COUNT(*),
            @showtime_id = MIN(showtime_id),
            @seat_total = SUM(price)
        FROM valid_holds;

        IF @hold_count IS NULL OR @hold_count = 0
        BEGIN
            RAISERROR(N'Danh sách ghế giữ không hợp lệ hoặc đã hết hạn.', 16, 1);
            ROLLBACK TRANSACTION;
            RETURN;
        END;
        ...
        IF EXISTS (
            SELECT 1
            FROM (
                SELECT ss.showtime_id
                FROM STRING_SPLIT(@hold_ids, ',') sp
                INNER JOIN seat_holds sh
                    ON TRY_CAST(sp.value AS INT) = sh.hold_id
...
-- Gợi ý săn vé theo form frontend
-- EXEC sp_find_ticket_suggestions
--     @movie_name = N'Doraemon',
...
-- Giữ thử 1 ghế cho user_id = 3
-- EXEC sp_hold_seat @user_id = 3, @showtime_seat_id = 1, @hold_minutes = 5;
-- GO

-- Tạo booking từ hold_id = 1
-- EXEC sp_create_booking_from_hold @user_id = 3, @hold_id = 1;
-- GO

-- Xác nhận thanh toán booking_id = 1
-- EXEC sp_confirm_payment 
--     @booking_id = 1,
--     @payment_method = 'MOMO',
--     @transaction_code = 'MOMO_TEST_001';
-- GO

-- Check-in vé
-- EXEC sp_checkin_ticket @ticket_code = 'TICKET_CODE_Ở_BẢNG_TICKETS';
-- GO