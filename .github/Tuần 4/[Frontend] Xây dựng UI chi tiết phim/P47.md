# [Frontend] Xây dựng UI chi tiết phim

> 🔗 Issue: [#47 — Minhhanh-zit/CNPM](https://github.com/Minhhanh-zit/CNPM/issues/47)
> 👤 Người phụ trách: **Cuaaaaa**
> 📌 Milestone: **Milestone 4 — Xây dựng Frontend**
> 🏷️ Labels: `frontend` · `booking` · `priority-high`
> 🔗 Thuộc issue mẹ: [#36](https://github.com/Minhhanh-zit/CNPM/issues/36)
> 📅 Tạo ngày: 04/06/2026
...
Xây dựng phần giao diện hiển thị thông tin chi tiết phim trong hệ thống đặt vé CMC Cinema.
...
|---|----------|-----------|---------|
| 1 | Hiển thị poster phim | ✅ Hoàn thành | Có fallback ảnh khi lỗi |
| 2 | Hiển thị tên phim | ✅ Hoàn thành | `text-white`, luôn rõ trên hero backdrop |
| 3 | Hiển thị mô tả phim | ✅ Hoàn thành | Fallback "Đang cập nhật..." nếu rỗng |
| 4 | Hiển thị thể loại | ✅ Hoàn thành | Badge từng genre dạng pill |
| 5 | Hiển thị thời lượng | ✅ Hoàn thành | Badge `{duration_minutes} phút` |
| 6 | Hiển thị độ tuổi phù hợp | ✅ Hoàn thành | Badge đỏ `{age_rating}` |
| 7 | Hiển thị trailer nếu có | ✅ Hoàn thành | Embed YouTube, fallback nút mờ nếu không có |
| 8 | Responsive giao diện | ✅ Hoàn thành | Tailwind breakpoints `md:` `lg:` đầy đủ |
...
| # | Tiêu chí | Trạng thái |
|---|----------|-----------|
| 1 | Trang chi tiết phim hiển thị đủ thông tin cơ bản | ✅ Đạt |
| 2 | Trailer hiển thị khi có dữ liệu | ✅ Đạt |
| 3 | Giao diện dễ nhìn và responsive | ✅ Đạt |
...
- **Navbar** — Header dùng chung toàn app
- **Hero Section**
  - Backdrop image với gradient overlay
  - Nút Quay lại / Trang chủ
  - Poster phim (có fallback)
  - Badges: `age_rating` · `duration` · `status`
  - Title — `text-white` luôn hiển thị rõ
  - Genre badges dạng pill
  - Description (fallback nếu rỗng)
  - CTA Buttons: Mua vé · Xem trailer · Yêu thích
- **Info Section** — 3 cột responsive
  - Thông tin phim: tên, thời lượng, phân loại, thể loại, trạng thái
  - Lịch chiếu gợi ý: 09:00 / 11:30 / 14:15 / 17:45 / 20:30
  - Hành động nhanh: Chọn ghế & mua vé / Xem suất chiếu gần nhất
- **Trailer Section**
  - `<iframe>` YouTube embed nếu có `trailer_url`
  - Thông báo "Chưa có trailer" nếu không có
- **Footer** — 3 cột: Thông tin liên hệ · Hỗ trợ · Về chúng tôi
...
function getYoutubeEmbedUrl(url?: string) {
  ...
  if (url.includes("watch?v=")) {
    const id = url.split("watch?v=")?.split("&");
    return id ? `https://www.youtube.com/embed/${id}` : null;
  }
  if (url.includes("youtu.be/")) {
    const id = url.split("youtu.be/")?.split("?");
    return id ? `https://www.youtube.com/embed/${id}` : null;
  }
  return null;
}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
            ...
            <div className={`rounded-2xl p-5 ${darkMode ? "bg-gray-900 border border-gray-800" : "bg-white shadow"}`}>
              ...
              <div className="flex flex-wrap gap-2">
                {["09:00", "11:30", "14:15", "17:45", "20:30"].map((time) => (
                  <button key={time} className="px-4 py-2 rounded-lg bg-red-500/10 text-red-400 border border-red-500/20 hover:bg-red-500 hover:text-white transition">
                    {time}
                  </button>
                ))}
              </div>
            ...
            <div className={`rounded-2xl p-5 ${darkMode ? "bg-gray-900 border border-gray-800" : "bg-white shadow"}`}>
              ...
              <div className="flex flex-col gap-3">
                ...
                <button className={`font-semibold px-5 py-3 rounded-xl border transition ${darkMode ? "bg-white/10 hover:bg-white/20 text-white border-white/10" : "bg-gray-100 hover:bg-gray-200 text-gray-900 border-gray-200"}`}>
...
| Routing | React Router v6 — `useParams`, `useNavigate`, `Link` |
| Styling | Tailwind CSS |
| Data | Mock data từ `mockMovies` |
| Trailer | YouTube embed qua `getYoutubeEmbedUrl()` |
| Fallback ảnh | `picsum.photos` cho poster/backdrop lỗi |
| Dark mode | `useState` local, nhận từ prop |

---

## 🎁 Bonus (ngoài yêu cầu)

- ✅ Header dùng chung `<Navbar />`
- ✅ Footer 3 cột thông tin
- ✅ Dark / Light mode toggle
- ✅ Fallback UI khi không tìm thấy phim
- ✅ Nút Mua vé, Yêu thích, Lịch chiếu, Chọn ghế

---

**✅ Kết luận: Đã hoàn thành 100% tiêu chí issue #47**
